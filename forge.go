...
func CheckForge(mcVersion string) bool {
	...
	forgeVersionDirPath := path.Join(getMinecraftBaseDir(), MinecraftDir[runtime.GOOS], MinecraftVersionsDir, forgeVersion)
...
func InstallForge(mcVersion string) {
	...
	libDir := path.Join(getMinecraftBaseDir(), MinecraftDir[runtime.GOOS], MinecraftLibrariesDir, ForgeLibDir, ForgeVersion[mcVersion])
