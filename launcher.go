...
func CreateProfile(mcVersion string, baseVersion string, suffix string, profileVersion string, ram int) {
	filePath := path.Join(getMinecraftBaseDir(), MinecraftDir[runtime.GOOS], "launcher_profiles.json")
	jsonFile, _ := os.Open(filePath)

	pdcDir := path.Join(getMinecraftBaseDir(), MinecraftDir[runtime.GOOS], PDCDir)
